...
class Model:
    ...
    def act(self, obs, dones, positions_xy, targets_xy) -> list:
        if self.agents is None:
            ...
            self.lst = [0 for i in range(self.lengthobs)]
            self.agents = [AStar() for _ in range(self.lengthobs)]  # create a planner for each of the agents
        actions = self.lst
        for k in range(self.lengthobs):
            if positions_xy[k] == targets_xy[k]:  # don't waste time on the agents that have already reached their goals
                actions[k]=0  # just add useless action to save the order and length of the actions
                continue
            ...
            actions[k]=self.actions[(next_node[0] - positions_xy[k][0], next_node[1] - positions_xy[k][1])]
        return actions
