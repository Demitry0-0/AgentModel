...
class AStar:
    ...
    def compute_shortest_path(self, start, goal, n=5):
        ...
        u = Node()
        steps = 0
        max_steps = min(max(self.max_steps - n * 12, 20), self.max_steps)
    ...
    def update_obstacles(self, obs, other_agents, n):
        ...
        for agent in agents:
            self.other_agents.add(
                (n[0] + agent[0], n[1] + agent[1]))  # save them with correct coordinates
...
class Model:
    ...
    def act(self, obs, dones, positions_xy, targets_xy) -> list:
        if self.agents is None:
            ...
            self.agents = [AStar() for _ in
                           range(self.lengthobs)]  # create a planner for each of the agents
        actions = []
        for k in range(self.lengthobs):
            if positions_xy[k] == targets_xy[
                k]:  # don't waste time on the agents that have already reached their goals
                actions.append(
                    0)  # just add useless action to save the order and length of the actions
                continue
...
def main():
    ...
    grid_config = GridConfig(num_agents=500,  # количество агентов на карте
                             size=64,  # размеры карты
                             density=0.3,  # плотность препятствий
                             seed=1,  # сид генерации задания
                             max_episode_steps=256,  # максимальная длина эпизода
                             obs_radius=5,  # радиус обзорg
                             )
